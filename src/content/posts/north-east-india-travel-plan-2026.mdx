...
flowchart LR
    ...
    SIL --> GHY["Guwahati\nAssam"]
    GHY --> SHL["Shillong · WFH\nMeghalaya"]
    SHL --> CHR["Cherrapunji\nMeghalaya"]
    CHR --> DWK["Dawki\nMeghalaya"]
    DWK --> GHY
    GHY --> TEZ["Tezpur\nAssam"]
    TEZ --> BHL["Bhalukpong · ILP\nArunachal P."]
    BHL --> BOM["Bomdila\nArunachal P."]
    BOM --> DIR["Dirang\nArunachal P."]
    DIR --> SEL["Sela Pass · 4170m\nArunachal P."]
    SEL --> TWG["Tawang\nArunachal P."]
    TWG -->|"return via\nSela Tunnel"| BOM
    BOM --> KZR["Kaziranga NP\nAssam"]
    KZR --> MAJ["Majuli\nAssam"]
    MAJ --> SIV["Sivasagar\nAssam"]
    SIV --> DBR["Dibrugarh\nAssam"]
    DBR --> ROI["Roing\nArunachal P."]
    ROI --> MAY["Mayudia · 2655m\nArunachal P."]
    MAY --> ANI["Anini\nArunachal P."]

    %% ─── ⚡ OPTIONAL — no extra days needed ────────────────────────
    SIL -.->|"⚡ +overnight\nDooars"| JLD(["Jaldapara NP\nWB Dooars"])
    GHY -.->|"⚡ Day 17\n+2 hrs"| POB(["Pobitora WLS\nAssam"])
    SHL -.->|"⚡ WFH\nmorning"| MWP(["Mawphlang\nSacred Forest"])
    CHR -.->|"⚡ Day 6"| KYN(["Kynrem Falls\nMeghalaya"])
    DWK -.->|"⚡ Day 16"| NAR(["Nartiang\nMonoliths"])
    BOM -.->|"⚡ Day 18"| THB(["Thembang\nFortified Village"])
    TWG -.->|"⚡ any\nTawang day"| GNK(["Gongkar La\n~4500m"])
    TWG -.->|"⚡ Day 21"| ZEM(["Zemithang\nValley"])
    DBR -.->|"⚡ Day 27"| DIG(["Digboi +\nDehing Patkai"])

    %% ─── ➕ EXTENSIONS — requires extra days ───────────────────────
    SIL -.->|"➕ +1–2d\nSikkim permit"| ZUL(["Zuluk · Silk Route\nSikkim"])
    GHY -.->|"➕ +1 day"| MAN(["Manas NP\nUNESCO WHS"])
...
**Legend:** 🟢 West Bengal &nbsp;·&nbsp; 🔵 Assam &nbsp;·&nbsp; 🟣 Meghalaya &nbsp;·&nbsp; 🔴 Arunachal Pradesh &nbsp;·&nbsp; 🟠 WFH base &nbsp;·&nbsp; 🟡 ⚡ Optional / easy add &nbsp;·&nbsp; ⬜ ➕ Requires extra days
